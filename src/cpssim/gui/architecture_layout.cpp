...
void normalize_architecture_workspace(GuiArchitectureWorkspace& state) noexcept {
    if (!std::isfinite(state.pan_x)) state.pan_x = 0.0F;
    if (!std::isfinite(state.pan_y)) state.pan_y = 0.0F;
...
void set_task_layout_position(GuiArchitectureWorkspace& state, TaskId id, GuiLayoutPoint position) {
    ...
    if (found != state.tasks.end()) found->position = position;
    else state.tasks.push_back({id, position});
}
...
void reset_resource_layout_position(GuiArchitectureWorkspace& state, ResourceId id) {
    ...
    row.position.reset();
    if (!row.size.has_value()) std::erase_if(state.resources, [id](const auto& value) { return value.resource_id == id; });
}
void reset_resource_layout_size(GuiArchitectureWorkspace& state, ResourceId id) {
    auto& row = resource_override(state, id);
    row.size.reset();
    if (!row.position.has_value()) std::erase_if(state.resources, [id](const auto& value) { return value.resource_id == id; });
}
