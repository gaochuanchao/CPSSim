...
Fmi2ModelInfo model_info(const std::filesystem::path& shared_library,
...
std::vector<GuiSignalDescriptor> bosch_signal_registry() {
    return {
        {{GuiSignalScalarType::Real, "lateral_error"}, "Bosch/Control/Lateral error",
         "Lateral error", "m", "Bosch FMU"},
        {{GuiSignalScalarType::Real, "actuator_command"}, "Bosch/Control/Actuator command",
         "Actuator command", "", "Bosch FMU"},
        {{GuiSignalScalarType::Real, "rolling_real"}, "Bosch/Timing/Rolling real",
         "Rolling real", "", "Bosch FMU"},
        {{GuiSignalScalarType::Real, "rolling_remote"}, "Bosch/Timing/Rolling remote",
         "Rolling remote", "", "Bosch FMU"},
        {{GuiSignalScalarType::Integer, "violation_counter"}, "Bosch/Timing/Violations",
         "Violation counter", "", "Bosch FMU"},
        {{GuiSignalScalarType::Boolean, "critical_section"}, "Bosch/Timing/Critical section",
         "Critical section", "", "Bosch FMU"},
    };
}

ProjectRuntimeInputs make_runtime(std::filesystem::path shared_library,
                                  std::vector<BoschTrajectorySample> trajectory,
                                  std::string instance_name) {
    auto samples = std::make_shared<const std::vector<BoschTrajectorySample>>(std::move(trajectory));
    auto info = model_info(shared_library, std::move(instance_name));
    return {.functional_model_factory = [info = std::move(info), samples] {
...
std::string serialize_settings(const BoschProjectSettings& settings) {
    if (settings.trajectory_directory.empty() || settings.trajectory_directory.is_absolute() ||
        settings.trajectory_directory.lexically_normal().begin() ==
            settings.trajectory_directory.lexically_normal().end() ||
        *settings.trajectory_directory.lexically_normal().begin() == "..") {
...
BoschProjectSettings load_settings(const std::filesystem::path& path) {
    ...
    if (!document.is_object() || document.size() != 3 ||
        !document.contains("schema_version") || !document.contains("trajectory_directory") ||
...
void validate_bosch_project_request(const BoschProjectRequest& request) {
    ...
    if (request.name.empty() || std::filesystem::path{request.name}.has_parent_path() ||
        request.name == "." || request.name == ".." || request.name.find('/') != std::string::npos ||
    ...
    if (request.shared_library.empty() || !std::filesystem::is_regular_file(request.shared_library)) {
...
ProjectRuntimeInputs resolve_bosch_project_runtime(
    ...
    if (metadata.scenario_kind != "bosch" || !metadata.scenario_file.has_value()) {
        throw std::invalid_argument{"unsupported project scenario kind: " +
                                    metadata.scenario_kind};
    }
